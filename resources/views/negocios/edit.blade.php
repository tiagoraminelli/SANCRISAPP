        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6">
            ...
            <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <span class="text-xs text-gray-400">Modifique los datos del comercio</span>
                </div>

                <form method="POST" action="{{ route('negocios.update', $negocio) }}" enctype="multipart/form-data" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-wrap border border-gray-200 rounded-xl overflow-hidden shadow-inner">
                        ...
                        <div class="w-full lg:w-3/5 p-6 border-r border-gray-200 bg-white space-y-5">
                            <div>
                                ...
                                <input type="text" name="nombre" value="{{ old('nombre', $negocio->nombre) }}" placeholder="Ej: Panadería La Estrella"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="horarios" value="{{ old('horarios', $negocio->horarios) }}" placeholder="Lun a Vie 9-18hs"
                                        class="w-full h-10 px-3 rounded-lg border-gray-200 text-sm focus:ring-black focus:border-black transition">
                                    @error('horarios')
                                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                                    @enderror
                            ...
                            {{-- Slug (Agregado para consistencia con create) --}}
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Slug (URL Amigable)</label>
                                <input type="text" name="slug" value="{{ old('slug', $negocio->slug) }}"
                                    class="w-full h-10 px-3 rounded-lg border-gray-200 text-sm bg-gray-50 focus:ring-black focus:border-black transition"
                                    placeholder="Se genera automáticamente">
                                <p class="text-[10px] text-gray-400 mt-1 uppercase">Dejar en blanco para autogenerar</p>
                                @error('slug')
                                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="w-full lg:w-2/5 p-6 bg-gray-50/50 space-y-5">
                            <div>
                                ...
                                @if($negocio->logo)
                                    <div class="mb-3 flex justify-center">
                                        <img src="{{ asset('storage/' . $negocio->logo) }}"
                                ...
                                <div class="border-2 border-dashed border-gray-200 p-4 bg-white text-center rounded-xl hover:border-black transition-colors group">
                                    <div class="py-2">
                                        ...
                                        <p class="text-[10px] text-gray-400 mt-1">Dejar vacío para mantener el actual</p>
                                    </div>
                            ...
                            <div>
                                ...
                                <textarea name="descripcion" rows="3"
                                    class="w-full border-gray-200 rounded-lg p-3 text-sm focus:ring-black focus:border-black resize-none">{{ old('descripcion', $negocio->descripcion) }}</textarea>
                                @error('descripcion')
                                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-gray-100 pt-6">
                        ...
                        <div class="flex items-center gap-3 bg-gray-50 px-4 py-2 rounded-xl border border-gray-100">
                            <input type="checkbox" name="activo" value="1"
                                   {{ old('activo', $negocio->activo) ? 'checked' : '' }}
